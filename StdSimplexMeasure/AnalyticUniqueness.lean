...
theorem analyticOnNhd_eq_of_eqOn_posReal {F G : ℂ → ℂ}
  ...
  let w : ℕ → ℂ := fun n ↦ ((1 + (n + 1 : ℝ)⁻¹ : ℝ) : ℂ)
  have hw : Tendsto w atTop (𝓝 (1 : ℂ)) := by
    have hr : Tendsto (fun n : ℕ ↦ 1 + (n + 1 : ℝ)⁻¹) atTop (𝓝 (1 : ℝ)) := by
      simpa using tendsto_const_nhds.add
        (tendsto_one_div_add_atTop_nhds_zero_nat (𝕜 := ℝ))
    change Tendsto (fun n : ℕ ↦ Complex.ofReal (1 + (n + 1 : ℝ)⁻¹)) atTop
      (𝓝 (Complex.ofReal 1))
    exact Complex.continuous_ofReal.continuousAt.tendsto.comp hr
  have hwne : ∀ n, w n ≠ 1 := by
    intro n h
    have : (1 + (n + 1 : ℝ)⁻¹ : ℝ) = 1 := Complex.ofReal_injective h
    have hinv : (0 : ℝ) < (n + 1 : ℝ)⁻¹ := by positivity
    linarith
  have hwp : ∀ n, F (w n) = G (w n) := by
    intro n
    apply hEq
    positivity
  apply hF.eq_of_frequently_eq hG (z₀ := 1)
  have hwithin : Tendsto w atTop (𝓝[≠] (1 : ℂ)) :=
    tendsto_nhdsWithin_iff.mpr ⟨hw, Eventually.of_forall hwne⟩
  exact hwithin.frequently (Frequently.of_forall hwp)
