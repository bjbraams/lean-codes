...
/-!
# Coordinate Lebesgue measure on the standard simplex
...
open MeasureTheory Fintype

public noncomputable section StdSimplexLebesgueMeasure
...
I have not introduced the distinction; all facts here are presented as `theorem`.
Raised on Zulip: <link once posted>.
-/
...
/-- The filled `(card ι - 1)`-dimensional simplex of free coordinates corresponding to points of
`stdSimplex ℝ ι`. (Not to be confused with `stdSimplex ℝ {j // j ≠ i}`.) -/
def stdSimplexFreeCoords (i : ι) :
    Set ({j : ι // j ≠ i} → ℝ) :=
  {x | (∀ j, 0 ≤ x j) ∧ ∑ j, x j ≤ 1}

/-- The preimage of `stdSimplex ℝ ι` under `stdSimplexCoordMap i` is `stdSimplexFreeCoords i`. -/
...
/-- The coordinate Lebesgue measure on `stdSimplexAffineSet`. When `ι` is empty the measure is
zero; otherwise it is the push-forward of Lebesgue measure from any set of free coordinates. -/
...
simplex measure. -/
theorem stdSimplexMeasure_restrict_stdSimplex [Nonempty ι] (i : ι) :
  stdSimplexMeasure.restrict (stdSimplex ℝ ι) =
...
theorem integral_stdSimplex_constant [Nonempty ι] :
    ∫ u in stdSimplex ℝ ι, (1 : ℝ) ∂stdSimplexMeasure =
      1 / (Nat.factorial (card ι - 1) : ℝ) := by
  sorry

/-- Equality between a Finsupp.prod and a full finite product for the same exponent vector. -/
