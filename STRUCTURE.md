...
| `ComplexAnalysis.Cycle.Parallelogram` | The boundary of a parallelogram, as a `C^∞` closed `Path`, built by gluing its four edges with `Real.smoothTransition` (all derivatives flat at the joins, so no case analysis on differentiability at the corners); the analytic index of this loop is `0` at every point outside the closed parallelogram (`curveIndex_parallelogramLoop_eq_zero`, a straight-line nullhomotopy coning the loop toward a vertex) and `1` at every point of the open interior (`curveIndex_parallelogramLoop_eq_one`, for `w1, w2` positively oriented) — each of the four edges' contribution to the real "turning" integral is computed as an explicit `Real.arctan` antiderivative, whose range bound pins the total to the unique multiple of `2π` in `(0, 4π)`, without tracking a branch of `Complex.log`/`Complex.arg` around the whole boundary |
| `ComplexAnalysis.EllipticResidue` | The boundary integral of a doubly periodic *continuous* function over a period parallelogram vanishes (`curveIntegral_toSpanSingleton_parallelogramLoop_eq_zero`): opposite edges cancel exactly, each being the periodic translate of the other traversed oppositely, made precise via an explicit reversal substitution and the gluing-function symmetry `Real.smoothTransition (1 - x) = 1 - Real.smoothTransition x` (`smoothTransition_one_sub`); combined with `Cycle.Parallelogram`'s index facts this is the elementary half of Liouville's second and third theorems for elliptic functions, but assembling the two (which needs weakening the continuity hypothesis from global to near-boundary, since a genuinely non-constant elliptic function has poles) is not done |
...
| `ComplexAnalysis.PringsheimVivanti` | **The Pringsheim–Vivanti theorem**: a power series `∑ a_n z^n` with nonnegative coefficients and finite radius of convergence `R` cannot be continued holomorphically across a neighborhood of the boundary point `R`, proved by showing such a continuation would force the series to converge at some real point beyond `R`; the proof Taylor-recenters the continuation at a point near `R`, computes the recentered coefficients as explicit nonnegative finite binomial sums `∑ₖ a_k C(k,n) x0^(k-n)` via term-by-term circle integration (Weierstrass's `M`-test) and the elementary evaluation of `∮ z^m dz`, and finishes with a finite double-sum comparison |
| `ComplexAnalysis.SokhotskiPlemelj` | **The Sokhotski–Plemelj jump relation**: for a density `φ(w) = ∑ₖ c_k w^k` (`k : ℤ`) on a circle `‖w‖ = R` given by an absolutely summable two-sided Laurent series, the interior and exterior Cauchy-type contour integrals `Cφ(z) = (2πi)⁻¹ ∮ φ(w)/(w-z) dw` agree with the explicit one-sided power series `cauchyTypeInterior c z = ∑ₙ c_n z^n` and `cauchyTypeExterior c z = -∑ₙ c_{-(n+1)} z^{-(n+1)}` on each open side, and their difference at any point of the circle recovers `φ` there; proved by identifying `c` with the actual Laurent coefficients of `φ` via term-by-term circle integration (reusing `PringsheimVivanti`'s elementary evaluation of `∮ z^m dz`) and the project's Laurent expansion (`hasSum_circleLaurentCoeff_nat` / `hasSum_circleLaurentCoeff_negSucc`) |
| `ComplexAnalysis.PaleyWiener` | **The Paley–Wiener theorem, holomorphic-extension half**: `paleyWienerTransform f τ z = ∫ t in -τ..τ, f t * exp (I z t)`, the Fourier-type transform of `f` extended by zero outside `[-τ, τ]`, extends holomorphically to an entire function (differentiation under the integral sign in the complex parameter, Mathlib's `hasDerivAt_integral_of_dominated_loc_of_deriv_le` with parameter space `H = ℂ`) of exponential type at most `τ` (an elementary bound on the kernel `exp (izt)` for `t ∈ [-τ, τ]`); the `L²`-membership and converse ("hard direction") halves of the classical theorem are not proved (see `COVERAGE.md`, item D) |
| `ComplexAnalysis.CircleReflection` | **Reflection across a circle**: on an open, inversion-invariant domain `U` avoiding the inversion pole `0` and the Möbius pole `-r`, a function holomorphic inside the circle `‖z‖ = r`, continuous up to it, and real-valued on it extends holomorphically across it (`analyticOnNhd_circleReflection`), by reduction to `Reflection`'s real-axis case via the Cayley-type Möbius map `cayleyCircle r z = I(r-z)/(r+z)`, whose imaginary part `(r² - ‖z‖²)/‖z+r‖²` (`im_cayleyCircle`) gives every region-mapping fact needed, and which conjugates circle inversion `z ↦ r²/conj z` to complex conjugation (`cayleyCircle_inv_conj`) |
| `ComplexAnalysis.ChordalMetric` | **The chordal metric** on `OnePoint ℂ`: `chordalDist p q` is the Euclidean distance between the images of `p, q` under inverse stereographic projection `stereographicInv : OnePoint ℂ → EuclideanSpace ℝ (Fin 3)` onto the unit sphere, giving every metric-space axiom (symmetry, triangle inequality via `dist_triangle`, definiteness via `stereographicInv_injective`) for free from the ambient Euclidean metric; the classical closed-form formula `chordalDist z w = 2‖z-w‖/(√(1+‖z‖²)√(1+‖w‖²))` (`chordalDist_coe_coe`) and the `z, ∞` case (`chordalDist_coe_infty`) are recovered as theorems |
| `ComplexAnalysis.SphericalDerivative` | The spherical derivative `sphericalDeriv f z = ‖f' z‖/(1+‖f z‖²)`, the local chordal-metric distortion factor of a holomorphic function, with its invariance under the inversion `w ↦ 1/w` (`sphericalDeriv_inv`), reflecting that inversion is a chordal isometry of the sphere |
