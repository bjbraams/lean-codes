...
theorem polynomial_R_relation_implies_L_relation {κ : Type*} (s : Finset κ)
    ...
    (hrel : ∀ a : ℂ, ∑ j ∈ s, (A j).eval (carlsonRecurrencePoint a b z) *
      regCarlsonR (-a + e j) (B j) z = 0) (a : ℂ) :
    ∑ j ∈ s, (A j).eval (carlsonRecurrencePoint a b z) * regCarlsonL (-a + e j) (B j) z =
      ∑ j ∈ s, (MvPolynomial.pderiv none (A j)).eval (carlsonRecurrencePoint a b z) *
        regCarlsonR (-a + e j) (B j) z := by
  have hd (j : κ) : HasDerivAt
      (fun w => (A j).eval (carlsonRecurrencePoint w b z) * regCarlsonR (-w + e j) (B j) z)
      ((MvPolynomial.pderiv none (A j)).eval (carlsonRecurrencePoint a b z) *
          regCarlsonR (-a + e j) (B j) z -
        (A j).eval (carlsonRecurrencePoint a b z) * regCarlsonL (-a + e j) (B j) z) a := by
    have hr := (hasDerivAt_regCarlsonR_L (-a + e j) (B j) hz).comp_of_eq a
  ...
  have heq : (fun w => ∑ j ∈ s, (A j).eval (carlsonRecurrencePoint w b z) *
      regCarlsonR (-w + e j) (B j) z) = fun _ : ℂ => 0 := funext hrel
  rw [heq] at h
...
theorem sum_carlsonAssociatedRecurrenceJointPolynomial_mul_regCarlsonL
    ...
    ∑ n ∈ Finset.range (Fintype.card ι + 1),
      ...
      ∑ n ∈ Finset.range (Fintype.card ι + 1),
        (MvPolynomial.pderiv none (carlsonAssociatedRecurrenceJointPolynomial n)).eval
          (carlsonRecurrencePoint a b z) * regCarlsonR (-a - n) b z := by
  simpa only [sub_eq_add_neg] using polynomial_R_relation_implies_L_relation
    ...
    (fun a => by simpa only [sub_eq_add_neg] using
      sum_carlsonAssociatedRecurrenceJointPolynomial_mul_regCarlsonR a b hz) a
