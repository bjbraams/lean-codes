...
private theorem exists_common_carlsonAssociated_parameters [Nonempty ι]
  ...
  let k : ι → ℕ := fun i => Finset.univ.sup (fun j => ((s j).parameter i).toNat)
  ...
  have hc : c ∈ mvBetaConvergent := by
    intro i
    have hi := hconv 0 i
    have hki : ((s 0).parameter i : ℝ) ≤ (k i : ℝ) := by exact_mod_cast hk 0 i
    change 0 < (b i + (k i : ℂ)).re
    change 0 < (b i + ((s 0).parameter i : ℂ)).re at hi
    simp only [add_re, natCast_re, intCast_re] at *
    linarith
...
theorem exists_polynomial_relation_associatedR
  ...
  | inr hι =>
    ...
    obtain ⟨N, B, hB, hT, hBT, hshift⟩ :=
      exists_common_carlsonAssociated_parameters t b s hconv
