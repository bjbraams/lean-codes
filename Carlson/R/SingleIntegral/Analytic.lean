...
variable {U : Set (κ → ℂ)} {a a' : (κ → ℂ) → ℂ} {b z : (κ → ℂ) → ι → ℂ}

/-- The integrand of the single integral, as a function of the parameter point and a complex
integration variable. -/
private def unitIntervalIntegrand (a a' : (κ → ℂ) → ℂ) (b z : (κ → ℂ) → ι → ℂ)
    (v : (κ → ℂ) × ℂ) : ℂ :=
  v.2 ^ (a v.1 - 1) * (1 - v.2) ^ (a' v.1 - 1) * ∏ i, ((1 - v.2) + v.2 * z v.1 i) ^ (-b v.1 i)

/-- Joint analyticity of the integrand at parameter points of `U` and interior points of the
unit interval. -/
private lemma analyticAt_unitIntervalIntegrand
    (ha : AnalyticOnNhd ℂ a U) (ha' : AnalyticOnNhd ℂ a' U)
    (hb : AnalyticOnNhd ℂ b U) (hz : AnalyticOnNhd ℂ z U)
    (hslit : ∀ p ∈ U, z p ∈ carlsonRSlitDomain) {p : κ → ℂ} (hp : p ∈ U)
    {u : ℝ} (hu : u ∈ Ioo 0 1) :
    AnalyticAt ℂ (unitIntervalIntegrand a a' b z) (p, (u : ℂ)) := by
...
/-- Continuity of the integrand in the real integration variable on the open unit interval. -/
private lemma continuousOn_unitIntervalIntegrand
    (ha : AnalyticOnNhd ℂ a U) (ha' : AnalyticOnNhd ℂ a' U)
    (hb : AnalyticOnNhd ℂ b U) (hz : AnalyticOnNhd ℂ z U)
    (hslit : ∀ p ∈ U, z p ∈ carlsonRSlitDomain) {p : κ → ℂ} (hp : p ∈ U) :
    ContinuousOn (fun u : ℝ => unitIntervalIntegrand a a' b z (p, (u : ℂ))) (Ioo 0 1) := by
  intro u hu
  exact ((analyticAt_unitIntervalIntegrand ha ha' hb hz hslit hp hu).continuousAt.comp_of_eq
    (show ContinuousAt (fun v : ℝ => (p, (v : ℂ))) u by fun_prop) rfl).continuousWithinAt

/-- Continuity in the integration variable of the parameter derivative of the integrand. -/
private lemma continuousOn_fderiv_unitIntervalIntegrand
    (ha : AnalyticOnNhd ℂ a U) (ha' : AnalyticOnNhd ℂ a' U)
    (hb : AnalyticOnNhd ℂ b U) (hz : AnalyticOnNhd ℂ z U)
    (hslit : ∀ p ∈ U, z p ∈ carlsonRSlitDomain) {p : κ → ℂ} (hp : p ∈ U) :
    ContinuousOn (fun u : ℝ =>
      fderiv ℂ (fun q => unitIntervalIntegrand a a' b z (q, (u : ℂ))) p) (Ioo 0 1) := by
  have heq (u : ℝ) (hu : u ∈ Ioo 0 1) :
      fderiv ℂ (fun q => unitIntervalIntegrand a a' b z (q, (u : ℂ))) p =
        (fderiv ℂ (unitIntervalIntegrand a a' b z) (p, (u : ℂ))).comp
          (ContinuousLinearMap.inl ℂ (κ → ℂ) ℂ) := by
    exact ((analyticAt_unitIntervalIntegrand ha ha' hb hz hslit
      hp hu).differentiableAt.hasFDerivAt.comp p
        (hasFDerivAt_prodMk_left (𝕜 := ℂ) p (u : ℂ))).fderiv
  apply ContinuousOn.congr _ heq
  intro u hu
  exact (((analyticAt_unitIntervalIntegrand ha ha' hb hz hslit hp hu).fderiv.continuousAt.comp_of_eq
    (show ContinuousAt (fun v : ℝ => (p, (v : ℂ))) u by fun_prop) rfl).clm_comp
      continuousAt_const).continuousWithinAt

/-- The node kernel of the integrand is continuous on a closed parameter ball inside `U`
times the closed unit interval. -/
private lemma continuousOn_unitIntervalKernel
    (hb : AnalyticOnNhd ℂ b U) (hz : AnalyticOnNhd ℂ z U)
    (hslit : ∀ p ∈ U, z p ∈ carlsonRSlitDomain) {p : κ → ℂ} {r : ℝ}
    (hball : Metric.closedBall p r ⊆ U) :
    ContinuousOn (fun v : (κ → ℂ) × ℝ => ∏ i, ((1 - v.2 : ℂ) + (v.2 : ℂ) * z v.1 i) ^ (-b v.1 i))
      (Metric.closedBall p r ×ˢ Icc 0 1) := by
  have hcoord (g : (κ → ℂ) → ι → ℂ) (hg : AnalyticOnNhd ℂ g U) (i : ι) :
      ContinuousOn (fun v : (κ → ℂ) × ℝ => g v.1 i) (Metric.closedBall p r ×ˢ Icc 0 1) :=
    (continuous_apply i).comp_continuousOn
      (hg.continuousOn.comp continuous_fst.continuousOn (fun v hv => hball hv.1))
  ...
  exact ((continuousOn_const.sub hu.continuousOn).add
    (hu.continuousOn.mul (hcoord z hz i))).cpow (hcoord b hb i |>.neg)
      (fun v hv => carlsonRSegment_mem_slitPlane (hslit v.1 (hball hv.1)) hv.2 i)

/-- On `(0, 1]` a power `x ^ (w - 1)` is dominated by the power with the smaller real part
of the exponent. -/
private lemma norm_cpow_sub_one_le_of_re_le {x : ℝ} (hx : 0 < x) (hx₁ : x ≤ 1) {v w : ℂ}
    (hvw : v.re ≤ w.re) : ‖(x : ℂ) ^ (w - 1)‖ ≤ ‖(x : ℂ) ^ (v - 1)‖ := by
...
/-- A local integrable majorant for the integrand near a parameter point: a beta density with
halved endpoint exponents times a uniform bound on the node kernel. -/
private lemma exists_locally_dominated_unitIntervalIntegrand
    (hU : IsOpen U) (ha : AnalyticOnNhd ℂ a U) (ha' : AnalyticOnNhd ℂ a' U)
    (hb : AnalyticOnNhd ℂ b U) (hz : AnalyticOnNhd ℂ z U)
    (hpos : ∀ p ∈ U, 0 < (a p).re ∧ 0 < (a' p).re)
    (hslit : ∀ p ∈ U, z p ∈ carlsonRSlitDomain) {p : κ → ℂ} (hp : p ∈ U) :
    ∃ (s : Set (κ → ℂ)) (bound : ℝ → ℝ), s ∈ 𝓝 p ∧
      Integrable bound (volume.restrict (Ioo (0 : ℝ) 1)) ∧
      ∀ᵐ u : ℝ ∂volume.restrict (Ioo (0 : ℝ) 1), ∀ q ∈ s,
        ‖unitIntervalIntegrand a a' b z (q, (u : ℂ))‖ ≤ bound u := by
  let A : ℂ := ((a p).re / 2 : ℝ)
  let B : ℂ := ((a' p).re / 2 : ℝ)
  have hA : 0 < A.re := by simpa [A] using half_pos (hpos p hp).1
  have hB : 0 < B.re := by simpa [B] using half_pos (hpos p hp).2
  have hevent : ∀ᶠ q in 𝓝 p, q ∈ U ∧ A.re < (a q).re ∧ B.re < (a' q).re := by
  ...
  have hkernel : ContinuousOn kernel (Metric.closedBall p r ×ˢ Icc 0 1) :=
    continuousOn_unitIntervalKernel hb hz hslit (fun q hq => (hball hq).1)
  ...
  intro q hq
  have h₀ := norm_cpow_sub_one_le_of_re_le hu.1 hu.2.le (v := A) (hball hq).2.1.le
  have h₁ := norm_cpow_sub_one_le_of_re_le (sub_pos.mpr hu.2) (by linarith [hu.1]) (v := B)
    (hball hq).2.2.le
  have hK : ‖kernel (q, u)‖ ≤ max C 0 :=
    (hC (mem_image_of_mem _ (show (q, u) ∈ Metric.closedBall p r ×ˢ Icc 0 1 from
      ⟨hq, hu.1.le, hu.2.le⟩))).trans (le_max_left _ _)
...
theorem analyticOnNhd_carlsonRUnitIntervalIntegral_comp
  ...
  change AnalyticOnNhd ℂ (fun p => ∫ u : ℝ, unitIntervalIntegrand a a' b z (p, (u : ℂ))
    ∂volume.restrict (Ioo (0 : ℝ) 1)) U
  apply analyticOnNhd_integral_of_locally_dominated hU
    (fun p hp => (continuousOn_unitIntervalIntegrand ha ha' hb hz hslit hp).aestronglyMeasurable
      measurableSet_Ioo)
    (fun p hp => (continuousOn_fderiv_unitIntervalIntegrand ha ha' hb hz hslit
      hp).aestronglyMeasurable measurableSet_Ioo)
  · filter_upwards [self_mem_ae_restrict measurableSet_Ioo] with u hu
    intro p hp
    exact (analyticAt_unitIntervalIntegrand ha ha' hb hz hslit hp hu).comp_of_eq (by fun_prop) rfl
  · intro p hp
    exact exists_locally_dominated_unitIntervalIntegrand hU ha ha' hb hz hpos hslit hp
