...
This file proves the polynomial and native-integral quadratic transformations of Carlson's
Sections 6.9 and 6.10. The first native identity follows by eliminating odd terms in the
series near equal nodes. The second follows Carlson's double-series proof on pp. 165–166:
absolute convergence and the Pochhammer convolution are supplied by `QuadraticSeries`.
Homogeneity and analytic continuation extend these local identities to the stated domains;
the second domain's two components are related by simultaneous negation of the square roots.

The complex-power forms here use explicit node and convergence hypotheses.
`QuadraticContinuation` and `EqualParameter` extend the parameter identities to the entire
regularizations and Carlson's ordinary parameter domain. Extending the node domains to
the book's larger branch-sensitive domains remains future work.
There are no admitted proofs in this file.
-/
...
private lemma SecondQuadraticDomain.affine {x y : ℂ} (hz : SecondQuadraticDomain x y)
...
private lemma SecondQuadraticDomain.same_sign {x y : ℂ} (hz : SecondQuadraticDomain x y) :
